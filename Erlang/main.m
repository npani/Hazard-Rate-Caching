...
% %                 disp('opt-cach');
% %                 disp(opt_cache);
%                 opt_temp= [opt_cache item];
% %                 disp('opt-temp');
% %                 disp(opt_temp);
%                 next_seq = arrivals(req_num+1:end,2).';
%                 loc= find(any(all(bsxfun(@eq,reshape(x.',1,n,[]),M),2),3));
%                 [Lio,Loca] = ismember(opt_temp,next_seq);
%                 Loca(~Loca) = Inf;
% %                 disp(Loca);
%                 [val, farthest_item] = max(Loca);
% %                 disp(farthest_item);
%                 if(Loca(end) ~= Inf && item ~= opt_temp(farthest_item))
% %                     disp(item);
%                     opt_cache(farthest_item) = item;
%                 end
% %                 disp('opt-cach-again');
% %                 disp(opt_cache);
%             else    
...
file_name = ['HR_results_Erlang_N' num2str(N) '.mat'];
...
print -depsc hit_prob_bound_opt_n5105_Erlang_ca.eps;
...
% 
% function farthest_item = get_index(arrivals_next, opt_cache, item)
...
%   end   
% end
% p2 = find_ele(arrivals_next, item);
% if(p2==-1)
%     farthest_item = -1; 
%     return;
% else
