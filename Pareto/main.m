...
%         arrival_times(item) = time + gprnd(k(item),sigma(item),theta(item));
        ...
        if(check_hit(last_hit,time,k,sigma,item, C(i)))
...
% file_name = ['HR_results_Pareto_N' num2str(N) '.mat'];
% load(file_name);
...
%h_legend = legend('Belady','Static','LRU', 'FIFO','Random');
set(h_legend,'FontSize',20, 'NumColumns',2);
print -dpdf hit_prob_bound_opt_n5105_Pareto_ca.pdf;
