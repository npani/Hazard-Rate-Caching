...
all_arrivals = [];
for n=1:N
    nth_arrivals = exprnd(1/p(n), 100, 1);

    while (sum(nth_arrivals) < num_requests)
        nth_arrivals = [nth_arrivals; exprnd(1/p(n), 100, 1)];
    end
...
file_name = ['HR_results_Poisson_N' num2str(N) '.mat'];
...
print -depsc hit_prob_bound_opt_n5105_Poisson_ca.eps;
