...
all_arrivals = [];
for n=1:N
    nth_arrivals = hyperrnd(p1,p2,lambda1(n),lambda2(n),100,1);

    while (sum(nth_arrivals) < num_requests)
        nth_arrivals = [nth_arrivals; hyperrnd(p1,p2,lambda1(n),lambda2(n),100,1)];
    end
...
%         arrival_times(item) = time + gprnd(k(item),sigma(item),theta(item));
        ...
        if(check_hit(last_hit,time,p1,p2,lambda1,lambda2,item, C(i)))
...
file_name = ['HR_results_Hyper_N' num2str(N) '.mat'];
...
print -depsc hit_prob_bound_opt_n5105_Hyper_ca.eps;
...
function sample = hyperrnd(p1,p2,l1,l2,lr,lc)
sample = exprnd(1/find_rate(p1,l1,l2),lr,lc);
end
